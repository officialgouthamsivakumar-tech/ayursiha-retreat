...
export default function FaqFormPage() {
  ...
  useEffect(() => {
    if (!isNew) {
      fetch(`/api/admin/faqs/${id}`)
        .then(r => r.json())
        .then(data => {
          setForm({ category: data.category ?? '', question: data.question ?? '', answer: data.answer ?? '', sort_order: data.sort_order ?? 0 })
          setLoading(false)
        })
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    else { setSuccess('Saved successfully.'); if (isNew) router.push(`/admin/faqs/${data.id}`) }
    setSaving(false)
  }
  ...
  if (loading) return (
    <>
      <div className="admin-topbar"><span className="admin-topbar-title">FAQ</span></div>
  ...
  return (
    <>
      <div className="admin-topbar">
        <span className="admin-topbar-title">{isNew ? 'New FAQ' : 'Edit FAQ'}</span>
      </div>
