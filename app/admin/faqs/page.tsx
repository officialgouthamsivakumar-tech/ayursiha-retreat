...
export default async function FaqsListPage() {
  let faqs: Record<string, string>[] = []
  try {
    const { data } = await supabaseAdmin.from('faqs').select('id,category,question,answer').order('category').order('sort_order')
    faqs = data ?? []
  } catch {}
  ...
  return (
    <>
      <div className="admin-topbar">
        <span className="admin-topbar-title">FAQs</span>
      </div>
        ...
        {faqs.length === 0 ? (
          <div className="admin-card">
            <div className="admin-empty">
              No FAQs yet. <Link href="/admin/faqs/new" style={{ color: '#2d7a4f' }}>Add the first one.</Link>
            </div>
          </div>
        ) : (
          categories.map(cat => (
                  ...
                  <tbody>
                    {faqs.filter(f => f.category === cat).map(f => (
                      <tr key={f.id}>
                        ...
                        <td>
                          <Link href={`/admin/faqs/${f.id}`} className="admin-btn admin-btn-ghost admin-btn-sm">Edit</Link>
                        </td>
                      </tr>
                    ))}
                  </tbody>
